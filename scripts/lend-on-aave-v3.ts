...
async function main() {
    ...
    const aaveWrapperAddress = getEnvVariable("ZEROX_AAVE_WRAPPER_ADDRESS")
    ...
    const aaveZeroXWrapper = contract.attach(aaveWrapperAddress)
    const supplyResult = await aaveZeroXWrapper.lend(
        lendToken,
        lendAmount,
        userAddress,
        {
